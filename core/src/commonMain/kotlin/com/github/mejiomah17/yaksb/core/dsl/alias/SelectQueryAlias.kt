...
class SelectQueryAlias<DRIVER_DATA_SOURCE, DRIVER_STATEMENT>(
    ...
    operator fun <V, DRIVER_DATA_SOURCE, DRIVER_STATEMENT> get(
        ...
        object : AliasableExpressionForCondition<V, DRIVER_DATA_SOURCE, DRIVER_STATEMENT> {
            override fun databaseType(): DatabaseType<V, DRIVER_DATA_SOURCE, DRIVER_STATEMENT> = expression.databaseType()

            override fun sql(): String = "$name.${expression.name}"
...
fun <DRIVER_DATA_SOURCE, DRIVER_STATEMENT> SelectQuery<DRIVER_DATA_SOURCE, DRIVER_STATEMENT>.`as`(
    name: String,
): SelectQueryAlias<DRIVER_DATA_SOURCE, DRIVER_STATEMENT> = SelectQueryAlias(query = this, name = name)
